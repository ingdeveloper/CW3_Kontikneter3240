    ...
    function (signalsConnector) {
        var ccwpump0 = function (params) {
            ...
            self.mStyle = ko.computed(function () {
                var self = this;
                var css = 'ccw-wf-weiche1-antriebAus';
                var cssGerade = '';
                var cssLinks = '';
                var cssRechts = '';
                ...
                var spule1_ein = ((value & 0x100) >> 8); //Befehl ein Richtung Spule 1 -> Rechts
                var hand = ((value & 0x200) >> 9);
                var spule2_ein = ((value & 0x400) >> 10); //Befehl ein Richtung Spule 2 -> Links
                ...
                var visibleGerade = false;

                var visibleLinks = false;
                var visibleRechts = false;

                var visibleSpuleGerade = false;
                ...
                if (self.linksStattRechts) {
                    visibleRechts = rmSpule1 || spule1_ein;
                    visibleSpuleRechts = spule1_ein;
                    visibleLinks = rmSpule2 || spule2_ein;
                    visibleSpuleLinks = spule2_ein;
                } else {
                    visibleLinks = rmSpule1 || spule1_ein || faehrtRichtungSpule1;
                    visibleSpuleLinks = spule1_ein;
                    visibleRechts = rmSpule2 || spule2_ein || faehrtRichtungSpule2;
                ...
                // Wenn Störung
                if (ohneSignal) {
                    css = 'ccw-wf-weiche1-ohneSignal';
                } else if ((stoerNeu != 0) || (stoer != 0)) {
                    if (stoerNeu != 0) {
                        css = 'ccw-wf-weiche1-antriebStoerNeu';
                    } else {
                        css = 'ccw-wf-weiche1-antriebStoer';
                    }
                    ...
                    if ((hand != 0) && (farbStatusHand == true)) {
                        if ((spule1_ein != 0) || (spule2_ein != 0)) {
                            if ((faehrtRichtungSpule1 != 0) || (faehrtRichtungSpule2 != 0)) {
                                css = 'ccw-wf-weiche1-lauf';
                            } else {
                                css = 'ccw-wf-weiche1-lauf';
                            }
                        } else {
                            css = 'ccw-wf-weiche1-antriebHand';
                        }
                        // Normalbetrieb
                    } else {
                        ...
                        if ((spule2_ein != 0) && (faehrtRichtungSpule2 != 0)) {
                            if (self.linksStattRechts) {
                                cssRechts = 'ccw-wf-weiche1-lauf';
                            } else {
                                cssLinks = 'ccw-wf-weiche1-lauf';
                            }
                        }
