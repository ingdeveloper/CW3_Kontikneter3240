define(['../../../services/connector', 'plugins/dialog', '../../../services/usersService', "../../../components/services/secured.service", 'src/viewmodels/cowi/dialoge/cwDialog'],
    function (signalsConnector, dialog, usersService, securedService, okCancelDialog) {
        ...
        var ctor = function (waageCSS) {
            ...
            self.reglerKneterAktiv = self.connector.getSignal('VMI3240DB56DBX02').value; //Teigtemperaturregler (0=Inaktiv, 1=Aktiv)   
            self.reglerKneter = self.connector.getSignal('VMI3240DB56DBX01').value; //Teigtemperaturregler (0=Ein, 1=Aus)   
            ...
            self.reglerTeigEinAus = ko.computed(function() {
                var self = this;
                var css1 = 'bg-default'
                var css2 = 'bg-default'
                var vorwahl = self.reglerKneter();
                if (vorwahl !== null) {
                    if (vorwahl != 0) {
                        css1 = 'btn-default';
                        css2 = 'btn-success';
            ...
            self.aliasNameByteZeilePumpe1 = {
                cssReiheRahmen: 'jj',
                bmk: '=01+H01-224TA5',
                kommentar: 'Umwälzpumpe Knetermantelkühlung',
                sollwert: '',
                istwert: '',
                lblState: {
                    lblTextDefault: '',
        ...
        ctor.prototype = {
            ...
            compositionComplete: function () {
                ...
                var svgUrl = "ContentCowi/svg/VMI3240_Solekuehlung_Kneter.svg";
                $.get(svgUrl)
                    .then(injectSvgSole)
            ...
            openDialogReglerAktiv: function(text) {
                var self = this;
                dialog.show(
                        new cwReglerAktiv(text)
                    )
                    .then(function(dialogResult) {
