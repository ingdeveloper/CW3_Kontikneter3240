 ...
 *  http://e-smartdev.com/
 * 
 *  Modified by WEBfactory GmbH, Klaus Boldt 2017 
 *  - updated to JQuery 1.7+ 
 *  - remove chrome passive listener warning
 *  - Tested on Desktop Chrome, Edge and Firefox 
 *  - Android Chrome and Android 5.2 Browser 
 *  - Firefox 52: Hybrid devices dont detect touch event 
 *  - Edge need property about:flags touch enable
 * 
    ...
    $.fn.smartZoom = function (method) {
        ...
        var publicMethods = {
            /**
              ...
              *                            'minScale' : 1 the min scale that will be applied on the zoom target
              ...
              *                            'initPosition' : '' add coordinates to current position by init or reset. e.g. {x: 100, y: 250}
              *                            'initScale': '', initial scale to zoom
              *                           } 
              */
            init: function (options) {
                ...
                targetElement.width(targetWidth);
                ...
                if ($("#" + zoomContainerId).length === 0 && targetElement && targetElement.parent) {
                    targetElement.parent().prepend(containerDiv);
                    containerDiv = targetElement.parent().find("#" + zoomContainerId);
                } else {
                    containerDiv = $("#" + zoomContainerId);
                }
                
                containerDiv.attr('style', targetElementInitialStyles);
                ...
                targetHeight = containerDiv.height() !== 0 ? containerDiv.height() : targetHeight;
                targetWidth = containerDiv.width() !== 0 ? containerDiv.width() : targetWidth;
                ...
                targetElement.data('smartZoomData', {
                    ...
                    originalSize: { width: targetWidth, height: targetHeight }, // plugin target size at the beginning
                    originalPosition: containerDiv.offset(), // plugin target position at the beginning 
                    ...
                    mouseWheelDeltaFactor: settings.mouseWheelDeltaFactor, // enable to slow down the zoom via mouse wheel
            ...
            zoom: function (scaleToAdd, globalRequestedPosition, duration) {
                ...
                // manage scale min, max smartData.adjustedPosInfos.scale
                newScale = Math.max(smartData.settings.minScale, newScale); // the scale couldn't be lowest than the initial scale
            ...
            pan: function (xToAdd, yToAdd, duration) {
                ...
                animate(targetElement, currentPosition.left + xToAdd, currentPosition.top + yToAdd, targetRect.width, targetRect.height, duration, function () { // set the new position and size via the animation function
        ...
        function mouseDownHandler(e) {
            //e.preventDefault(); //the browser does not pass the focus event to the control resulting the behavior that the inputs are not accepting input after click selection
        ...
        function moveOnMotion(xPos, yPos, duration, motionEnd) {
            ...
            animate(targetElement, newMarginLeft, newMarginTop, targetRect.width, targetRect.height, duration, motionEnd == true ? function () {
                dispatchSmartZoomEvent(ESmartZoomEvent.PAN, ESmartZoomEvent.END, false);
        ...
        function applyInitOptions() {
            ...
            if (smartData.settings.initScale)
                publicMethods.zoom(smartData.settings.initScale - currentScale);

            if (ko.unwrap(smartData.settings.initPosition) !== undefined) {
                var initPosition = ko.unwrap(smartData.settings.initPosition);

                var x = ko.unwrap(initPosition.x) !== undefined
                            ? !isNaN(ko.unwrap(initPosition.x)) ? ko.unwrap(initPosition.x) : 0
                            : 0;

                var y = ko.unwrap(initPosition.y) !== undefined
                            ? !isNaN(ko.unwrap(initPosition.y)) ? ko.unwrap(initPosition.y) : 0
                            : 0;

                publicMethods.pan(x, y);
            }
        ...
        function animate(target, globalLeft, globalTop, width, height, duration, callback) {
            ...
            if (smartData.transitionObject != null) { // use css transition if supported
                ...
                $(target).trigger("scaleChanged",
                    {
                        x: width / originalSize.width,
                        y: height / originalSize.height
                    });
        ...
        function animateTO(target, coodsX, coodsY, scale, width, height, duration, callback) {
            ...
            if (smartData.transitionObject != null) { // use css transition if supported
                ...
                target.animate({ "margin-left": coodsX, "margin-top": coodsY, "width": width, "height": height }, {
                    duration: duration, easing: smartData.settings.easing, complete: function () {
                        if (callback != null)
                            callback();
                    }
