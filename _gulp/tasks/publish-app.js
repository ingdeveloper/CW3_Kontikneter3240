...
function publishFiles(destDir, callback) {
    let glob = [
        ...
        `${paths.app.dist}/release-app.js`,
        `${paths.fonts}/**/*.*`,
        `${paths.font}/**/*.*`,
        `${paths.content}/**/{(*.*),@(*.bundle.css),!(*.css)}`,
        `${paths.contentDemos}/**/{(*.*),@(*.bundle.css),!(*.css)}`,
        `${paths.contentCowi}/**/{(*.*),@(*.bundle.css),!(*.css)}`, //neu eingefügt am 5.04.2018
        // `${paths.node}/durandal/**/*.*`
    ];
...
function publishIndex(destDir, callback) {
    gulp.src("index-release-app.html")
...
function publishWebConfig(destDir, callback) {
    gulp
        .src("Web.config")
        .pipe(debug())
        .pipe(gulp.dest(destDir))
...
gulp.task("publish-app",
    ["build-app-bundles", "build-content-bundles", "build-script-bundles", "build-index", "set-index", "clean-app"],
    (cb) => {
        ...
        var streams = [
            ...
            (callback) => publishWebConfig(destDir, callback),
